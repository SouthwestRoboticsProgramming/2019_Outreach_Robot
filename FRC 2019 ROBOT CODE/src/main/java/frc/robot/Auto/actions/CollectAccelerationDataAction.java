...
public class CollectAccelerationDataAction implements Action {
    ...
    @Override
    public void start() {
        Robot.driveSubsystem.setOpenLoop(new DriveSignal((mReverse ? -1.0 : 1.0) * kStartPower, (mReverse ? -1.0 : 1.0) * (mTurn ? -1.0 : 1.0) * kStartPower));
    ...
    public void update() {
        ...
        double currentTime;
        synchronized (mDrive) {
            currentVelocity = Robot.driveSubsystem.getAverageDriveVelocityMagnitude() / 6; // rad/s
            currentTime = Robot.driveSubsystem.getTimestamp();
        }
